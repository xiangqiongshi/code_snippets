...
class MatMulOp(Op):
    ...
    def gradient(self, node, output_grad):
        ...
        """
        """TODO: Your code here"""
        nodeA = MatMulOp.__call__(self, output_grad, node.inputs[1], node.matmul_attr_trans_A, not node.matmul_attr_trans_B)
        nodeB = MatMulOp.__call__(self, node.inputs[0], output_grad, not node.matmul_attr_trans_A, node.matmul_attr_trans_B)
        return [nodeA, nodeB]
...
def gradients(output_node, node_list):
    ...
    #         node_to_output_grads_list[node] = zeroslike_op(node)
    ...
    for node in reverse_topo_order:
        if node in node_to_output_grads_list:
            if len(node.inputs) > 0:
                grads = node.op.gradient(node, node_to_output_grads_list[node])
                for n, gr in zip(node.inputs, grads):
                    if n in node_to_output_grads_list:
                        node_to_output_grads_list[n] = node_to_output_grads_list[n]+gr
                    else:
                        node_to_output_grads_list[n] = gr
